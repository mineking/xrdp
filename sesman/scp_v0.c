...
scp_v0_process(struct SCP_CONNECTION *c, struct SCP_SESSION *s) {
	...
	if (s->type == SCP_CHECK_IF_SESSION_EXISTS) {
		s_item = session_get_bydata(s->username, s->width, s->height, s->bpp,
				s->type, s->client_ip);

		if (s_item != 0) {
			log_info("check session : session exists");
			scp_v0s_reply_session_exists(c, 1);
		} else {
			log_info("check session : no previous session found  ");
			scp_v0s_reply_session_exists(c, 0);
		}

	} else if (s->type == SCP_GW_AUTHENTICATION) {
		...
		if (data) {
			if (1 == access_login_allowed(s->username)) {
				...
				log_message(LOG_LEVEL_INFO,
						"Username okey but group problem for "
								"user: %s", s->username);
			...
			log_message(LOG_LEVEL_INFO,
					"Username or password error for user: %s", s->username);
			scp_v0s_replyauthentication(c, errorcode);
		}

		auth_end(data);
	} else if (data) {
		log_info("session_get_bydata");
		s_item = session_get_bydata(s->username, s->width, s->height, s->bpp,
				s->type, s->client_ip);

		if (s_item != 0 && s->startNewSession == 0) {
			display = s_item->display;
			log_info("existing session");
			if (0 != s->client_ip) {
				log_message(LOG_LEVEL_INFO,
						"++ reconnected session: username %s, "
								"display :%d.0, session_pid %d, ip %s",
						s->username, display, s_item->pid, s->client_ip);
			} else {
				log_message(LOG_LEVEL_INFO,
						"++ reconnected session: username %s, "
								"display :%d.0, session_pid %d", s->username,
						display, s_item->pid);
			}
			...
			if (s_item != 0 && s->startNewSession != 0) {
				// kill old session
				log_info("killing session pid:%ld", s_item->pid);
				session_kill(s_item->pid);
			}
			...
			if (1 == access_login_allowed(s->username)) {
				...
				if (SCP_SESSION_TYPE_XVNC == s->type) {
					...
					display = session_start(s->width, s->height, s->bpp,
							s->username, s->password, data,
							SESMAN_SESSION_TYPE_XVNC, s->domain, s->program,
							s->directory, s->client_ip);
				} else if (SCP_SESSION_TYPE_XRDP == s->type) {
					log_message(LOG_LEVEL_INFO, "starting X11rdp session...");
					display = session_start(s->width, s->height, s->bpp,
							s->username, s->password, data,
							SESMAN_SESSION_TYPE_XRDP, s->domain, s->program,
							s->directory, s->client_ip);
					...
					display = session_start(s->width, s->height, s->bpp,
							s->username, s->password, data,
							SESMAN_SESSION_TYPE_XORG, s->domain, s->program,
							s->directory, s->client_ip);
				}
			} else {
				display = 0;
