...
scp_session_create() {
	...
	if (0 == s) {
		log_message(LOG_LEVEL_WARNING,
				"[session:%d] session create: malloc error", __LINE__);
		return 0;
	}
...
int scp_session_set_type(struct SCP_SESSION *s, tui8 type) {
	...
	case SCP_SESSION_TYPE_MANAGE:
		...
		s->mng = (struct SCP_MNG_DATA *) g_malloc(sizeof(struct SCP_MNG_DATA),
				1);

		if (NULL == s->mng) {
			log_message(LOG_LEVEL_ERROR,
					"[session:%d] set_type: internal error", __LINE__);
			return 1;
		}
	...
	default:
		log_message(LOG_LEVEL_ERROR, "[session:%d] set_type: unknown type",
		__LINE__);
		return 1;
	}
...
int scp_session_set_version(struct SCP_SESSION *s, tui32 version) {
	...
	default:
		log_message(LOG_LEVEL_WARNING,
				"[session:%d] set_version: unknown version", __LINE__);
		return 1;
	}
...
int scp_session_set_locale(struct SCP_SESSION *s, char *str) {
	if (0 == str) {
		log_message(LOG_LEVEL_WARNING, "[session:%d] set_locale: null locale",
		__LINE__);
		s->locale[0] = '\0';
		return 1;
	}
...
int scp_session_set_username(struct SCP_SESSION *s, char *str) {
	if (0 == str) {
		log_message(LOG_LEVEL_WARNING,
				"[session:%d] set_username: null username", __LINE__);
		return 1;
	}
	...
	if (0 == s->username) {
		log_message(LOG_LEVEL_WARNING,
				"[session:%d] set_username: strdup error", __LINE__);
		return 1;
	}
...
int scp_session_set_password(struct SCP_SESSION *s, char *str) {
	if (0 == str) {
		log_message(LOG_LEVEL_WARNING,
				"[session:%d] set_password: null password", __LINE__);
		return 1;
	}
	...
	if (0 == s->password) {
		log_message(LOG_LEVEL_WARNING,
				"[session:%d] set_password: strdup error", __LINE__);
		return 1;
	}
...
int scp_session_set_domain(struct SCP_SESSION *s, char *str) {
	if (0 == str) {
		log_message(LOG_LEVEL_WARNING, "[session:%d] set_domain: null domain",
		__LINE__);
		return 1;
	}
	...
	if (0 == s->domain) {
		log_message(LOG_LEVEL_WARNING, "[session:%d] set_domain: strdup error",
		__LINE__);
		return 1;
	}
...
int scp_session_set_program(struct SCP_SESSION *s, char *str) {
	if (0 == str) {
		log_message(LOG_LEVEL_WARNING, "[session:%d] set_program: null program",
		__LINE__);
		return 1;
	}
	...
	if (0 == s->program) {
		log_message(LOG_LEVEL_WARNING, "[session:%d] set_program: strdup error",
		__LINE__);
		return 1;
	}
...
int scp_session_set_directory(struct SCP_SESSION *s, char *str) {
	if (0 == str) {
		log_message(LOG_LEVEL_WARNING,
				"[session:%d] set_directory: null directory", __LINE__);
		return 1;
	}
	...
	if (0 == s->directory) {
		log_message(LOG_LEVEL_WARNING,
				"[session:%d] set_directory: strdup error", __LINE__);
		return 1;
	}
...
int scp_session_set_client_ip(struct SCP_SESSION *s, char *str) {
	if (0 == str) {
		log_message(LOG_LEVEL_WARNING, "[session:%d] set_client_ip: null ip",
		__LINE__);
		return 1;
	}
	...
	if (0 == s->client_ip) {
		log_message(LOG_LEVEL_WARNING,
				"[session:%d] set_client_ip: strdup error", __LINE__);
		return 1;
	}
...
int scp_session_set_hostname(struct SCP_SESSION *s, char *str) {
	if (0 == str) {
		log_message(LOG_LEVEL_WARNING,
				"[session:%d] set_hostname: null hostname", __LINE__);
		return 1;
	}
	...
	if (0 == s->hostname) {
		log_message(LOG_LEVEL_WARNING,
				"[session:%d] set_hostname: strdup error", __LINE__);
		return 1;
	}
...
int scp_session_set_errstr(struct SCP_SESSION *s, char *str) {
	if (0 == str) {
		log_message(LOG_LEVEL_WARNING, "[session:%d] set_errstr: null string",
		__LINE__);
		return 1;
	}
	...
	if (0 == s->errstr) {
		log_message(LOG_LEVEL_WARNING, "[session:%d] set_errstr: strdup error",
		__LINE__);
		return 1;
	}
...
int scp_session_set_addr(struct SCP_SESSION *s, int type, void *addr) {
	...
	case SCP_ADDRESS_TYPE_IPV4:
		...
		if (ret == 0) {
			log_message(LOG_LEVEL_WARNING,
					"[session:%d] set_addr: invalid address", __LINE__);
