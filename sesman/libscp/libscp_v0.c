...
/* client API */
/******************************************************************************/
enum SCP_CLIENT_STATES_E scp_v0c_connect(struct SCP_CONNECTION *c,
	...
	if (s->type == SCP_SESSION_TYPE_XVNC) {
		...
		log_message(LOG_LEVEL_WARNING,
				"[v0:%d] connection aborted: network error", __LINE__);
	...
	if (0
			!= scp_tcp_force_send(c->in_sck, c->out_s->data,
					c->out_s->end - c->out_s->data)) {
		log_message(LOG_LEVEL_WARNING,
				"[v0:%d] connection aborted: network error", __LINE__);
	...
	if (0 != version) {
		log_message(LOG_LEVEL_WARNING,
				"[v0:%d] connection aborted: version error", __LINE__);
	...
	if (size < 14) {
		log_message(LOG_LEVEL_WARNING,
				"[v0:%d] connection aborted: packet size error", __LINE__);
	...
	if (3 != sz) {
		log_message(LOG_LEVEL_WARNING,
				"[v0:%d] connection aborted: sequence error", __LINE__);
	...
	if (1 != sz) {
		log_message(LOG_LEVEL_WARNING,
				"[v0:%d] connection aborted: connection denied", __LINE__);
...
/* server API */
/******************************************************************************/
enum SCP_SERVER_STATES_E scp_v0s_accept(struct SCP_CONNECTION *c,
		struct SCP_SESSION **s, int skipVchk) {
	tui32 version = 0;
	tui32 size;
	...
	if (code
			== 0|| code == 10 || code == 20 || code==SCP_CHECK_IF_SESSION_EXISTS) {
		...
		} else if (code == SCP_CHECK_IF_SESSION_EXISTS) {
			scp_session_set_type(session, SCP_CHECK_IF_SESSION_EXISTS);
		}
		...
		if (0 != scp_session_set_username(session, buf)) {
			...
			log_message(LOG_LEVEL_WARNING,
					"[v0:%d] connection aborted: error setting username",
					__LINE__);
			return SCP_SERVER_STATE_INTERNAL_ERR;
		}
		...
		if (0 != scp_session_set_password(session, buf)) {
			scp_session_destroy(session);
			log_message(LOG_LEVEL_WARNING,
					"[v0:%d] connection aborted: error setting password",
					__LINE__);
			return SCP_SERVER_STATE_INTERNAL_ERR;
		}
...
/******************************************************************************/
enum SCP_SERVER_STATES_E scp_v0s_allow_connection(struct SCP_CONNECTION *c,
SCP_DISPLAY d) {
...
/******************************************************************************/
enum SCP_SERVER_STATES_E scp_v0s_replyauthentication(struct SCP_CONNECTION *c,
...
enum SCP_SERVER_STATES_E scp_v0s_reply_session_exists(struct SCP_CONNECTION *c,
		unsigned short int exists) {
	log_info("reply exists : %d", exists);
	out_uint32_be(c->out_s, 0); /* version */
	out_uint32_be(c->out_s, 14); /* size */
	/* cmd SCP_GW_AUTHENTICATION means authentication reply */
	out_uint16_be(c->out_s, SCP_CHECK_IF_SESSION_EXISTS);
	out_uint16_be(c->out_s, exists); /* reply code  */
	out_uint16_be(c->out_s, 0); /* dummy data */
	s_mark_end(c->out_s);
