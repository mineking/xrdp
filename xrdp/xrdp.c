...
main(int argc, char **argv) {
	...
	char text[256];
	char pid_file[256];
	log_info("starting rdp-server");log_info("starting rdp-server");
	g_init("xrdp");
	ssl_init();
...
//    }
...
#endif
	{
		log_error("endian wrong, edit arch.h");
		return 0;
	}

	/* check long, int and void* sizes */
	if (sizeof(int) != 4) {
		log_error("unusable int size, must be 4");
		return 0;
	}

	if (sizeof(long) != sizeof(void *)) {
		log_error("long size must match void* size");
		return 0;
	}

	if (sizeof(long) != 4 && sizeof(long) != 8) {
		log_error("unusable long size, must be 4 or 8");
		return 0;
	}

	if (sizeof(tui64) != 8) {
		log_error("unusable tui64 size, must be 8");
		return 0;
	}

	g_snprintf(cfg_file, 255, "%s/xrdp.ini", XRDP_CFG_PATH);

	startup_params = (struct xrdp_startup_params *) g_malloc(
			sizeof(struct xrdp_startup_params), 1);
	...
	log_info("reading configuration file %s", cfg_file);
	error = log_start(cfg_file, "XRDP");

	if (error != LOG_STARTUP_OK) {
		switch (error) {
		case LOG_ERROR_MALLOC:
			log_error("error on malloc. cannot start logging. quitting.");
			break;
		case LOG_ERROR_FILE_OPEN:
			log_error("error opening log file [%s]. quitting.",
					getLogFile(text, 255));
			break;
		default:
			log_error("log_start error");
			break;
		}
	...
	if (g_sync_event == 0) {
		log_error("error creating g_sync_event");
	}
