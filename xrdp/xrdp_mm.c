...
xrdp_mm_create(struct xrdp_wm *owner) {
	...
	LLOGLN(0,
			("xrdp_mm_create: bpp %d mcs_connection_type %d " "jpeg_codec_id %d v3_codec_id %d rfx_codec_id %d " "h264_codec_id %d", self->wm->client_info->bpp, self->wm->client_info->mcs_connection_type, self->wm->client_info->jpeg_codec_id, self->wm->client_info->v3_codec_id, self->wm->client_info->rfx_codec_id, self->wm->client_info->h264_codec_id));
...
xrdp_mm_module_cleanup(struct xrdp_mm *self) {
	...
	if (self->wm->hide_log_window) {
		...
		xrdp_wm_set_login_mode(self->wm, LOGIN_MODE_INITIALISING_LOGIN_WINDOW); /* reset session */
	}
...
static int APP_CC
xrdp_mm_send_login(struct xrdp_mm *self, int checkOnly, int startNewSession) {
	struct stream *s;
	int rv;
	int index;
	int count;
	...
	if (checkOnly) {
		self->code = SCP_CHECK_IF_SESSION_EXISTS;
...
xrdp_mm_setup_mod1(struct xrdp_mm *self) {
	...
	if (self->mod_handle == 0) {
		...
		if (self->mod_handle != 0) {
			...
			if (func == 0) {
				g_snprintf(text, 255,
						"error finding proc mod_init in %s, not a valid "
								"xrdp backend", lib);
				xrdp_wm_log_msg(self->wm, text);
			...
			if (func == 0) {
				g_snprintf(text, 255,
						"error finding proc mod_exit in %s, not a valid "
								"xrdp backend", lib);
				xrdp_wm_log_msg(self->wm, text);
				log_message(LOG_LEVEL_ERROR, text);
			}
			...
			if ((self->mod_init != 0) && (self->mod_exit != 0)) {
				...
				if (self->mod != 0) {
					g_writeln(
							"loaded module '%s' ok, interface size %d, version %d",
							lib, self->mod->size, self->mod->version);
				}
			} else {
				log_message(LOG_LEVEL_ERROR,
						"no mod_init or mod_exit address found");
			}
		} else {
			g_snprintf(text, 255,
					"error loading %s specified in xrdp.ini, please "
							"add a valid entry like lib=libxrdp-vnc.so or similar",
					lib);
			xrdp_wm_log_msg(self->wm, text);
			log_message(LOG_LEVEL_ERROR, text);
			return 1;
		...
		if (self->mod != 0) {
			...
			self->mod->server_create_os_surface_bpp =
					server_create_os_surface_bpp;
	...
	if (self->mod == 0) {
		log_error("problem loading library");
		return 1;
	}
...
xrdp_mm_process_rail_update_window_text(struct xrdp_mm* self, struct stream* s) {
	...
	g_writeln("  set window title %s size %d 0x%8.8x", rwso.title_info, size,
			flags);
	rv = libxrdp_orders_init(self->wm->session);
	rv = libxrdp_window_new_update(self->wm->session, window_id, &rwso, flags);
	rv = libxrdp_orders_send(self->wm->session);
...
xrdp_mm_chan_process_msg(struct xrdp_mm *self, struct trans *trans,
	...
	while (s_check_rem(s, 8)) {
		...
		default:
			log_message(LOG_LEVEL_ERROR,
					"xrdp_mm_chan_process_msg: unknown id %d", id);
			break;
		}
...
static void cleanup_sesman_connection(struct xrdp_mm *self) {
	...
	if (self->wm->login_mode != LOGIN_MODE_CLOSING
			&& self->wm->login_mode != LOGIN_MODE_INITIALISING_SESSION_WINDOW && self->wm->login_mode != LOGIN_MODE_CONNECTING_TO_DISPLAY) {
		log_info("closing because login mode is %d", self->wm->login_mode);
		xrdp_wm_set_login_mode(self->wm, LOGIN_MODE_CLOSED);
		xrdp_mm_module_cleanup(self);
	}
}

static int APP_CC
xrdp_mm_process_session_exists_response(struct xrdp_mm *self, struct stream *s) {
	int exists;
	int zero;
	int result = 0;
	in_uint16_be(s, exists);
	in_uint16_be(s, zero);
	if (zero != 0) {
		log_error("non zero data");
		result = 1;
	} else {
		if (exists) {
			log_info("a session already exists");
			xrdp_wm_set_login_mode(self->wm,
			LOGIN_MODE_INITIALISING_SESSION_WINDOW);
		} else {
			log_info("no previous session found");
			xrdp_wm_set_login_mode(self->wm, LOGIN_MODE_CONNECTING_TO_DISPLAY);
		}
	}
	...
	if (ok) {
		...
		if (xrdp_mm_setup_mod1(self) == 0) {
			if (xrdp_mm_setup_mod2(self) == 0) {
				...
				xrdp_wm_set_login_mode(self->wm, LOGIN_MODE_CLOSING);
				self->wm->dragging = 0;
...
xrdp_mm_sesman_data_in(struct trans *trans) {
	...
	log_info("got answer from sesman");
	self = (struct xrdp_mm *) (trans->callback_data);
	s = trans_get_in_s(trans);
	...
	if (error == 0) {
		in_uint16_be(s, code);
		log_info("got answer from sesman code: %d", code);
		switch (code) {

		case 5: // session exists?
			error = xrdp_mm_process_session_exists_response(self, s);
			break;
		case 3:/* even when the request is denied the reply will hold 3 as the command. */
			error = xrdp_mm_process_login_response(self, s);
			break;
		default:
			xrdp_wm_log_msg(self->wm,
					"An undefined reply code was received from sesman");
			log_message(LOG_LEVEL_ERROR,
					"Fatal xrdp_mm_sesman_data_in: unknown cmd code %d", code);
			cleanup_sesman_connection(self);
			break;
		}
	}
...
access_control(char *username, char *password, char *srv) {
	...
	if (socket != -1) {
				...
				if (g_tcp_can_recv(socket, 5000)) {
					...
					if (reply > 0) {
						...
						if ((size == 14) && (version == 0)) {
							...
							log_message(LOG_LEVEL_ERROR,
									"Corrupt reply size or "
											"version from sesman: %d", size);
						}
					} else {
						log_message(LOG_LEVEL_ERROR,
								"No data received from sesman");
					}
				} else {
					log_message(LOG_LEVEL_ERROR,
							"Timeout when waiting for sesman");
				}
			} else {
		...
		log_message(LOG_LEVEL_ERROR,
				"Failure creating socket - for access control");
	}
...
/*****************************************************************************/
int APP_CC
xrdp_mm_connect(struct xrdp_mm *self, int checkOnly, int starNewSession) {
struct list *names;
struct list *values;
int index;
int count;
...
if (!g_strncmp(pam_auth_username, "same", 255)) {
	log_message(LOG_LEVEL_DEBUG, "pamusername copied from username - same: %s",
			username);
...
if (!g_strncmp(pam_auth_password, "same", 255)) {
	log_message(LOG_LEVEL_DEBUG,
			"pam_auth_password copied from username - same: %s", password);
...
log_info("login is controlled by sesman");
ok = 0;
trans_delete(self->sesman_trans);
...
for (index = 0; index < 4; index++) {
	...
	g_sleep(1000);
	log_error("xrdp_mm_connect: connect failed " "trying again...");
}
...
if (ok) {
	...
	rv = xrdp_mm_send_login(self, checkOnly, starNewSession);
	if (checkOnly) {
		return rv;
	}
} else {
	g_snprintf(errstr, 255, "Failure to connect to sesman: %s port: %s", ip,
			port);
	log_error("Failure to connect to sesman: %s port: %s", ip, port);
...
if (xrdp_mm_setup_mod1(self) == 0) {
	if (xrdp_mm_setup_mod2(self) == 0) {
		...
		log_error("Failure to connect to : %s", ip);
		/* connect error */
...
log_info("login mode : %d", self->wm->login_mode);
...
/*****************************************************************************/
int APP_CC
xrdp_mm_get_wait_objs(struct xrdp_mm *self, tbus *read_objs, int *rcount,
...
if (self->mod->mod_get_wait_objs != 0) {
	rv = self->mod->mod_get_wait_objs(self->mod, read_objs, rcount, write_objs,
			wcount, timeout);
}
}
...
if (g_is_wait_obj_set(self->encoder->xrdp_encoder_event_processed)) {
	...
	enc_done = (XRDP_ENC_DATA_DONE*) fifo_remove_item(
			self->encoder->fifo_processed);
	tc_mutex_unlock(self->encoder->mutex);
	while (enc_done != 0) {
		/* do something with msg */
		LLOGLN(10,
				("xrdp_mm_check_wait_objs: message back bytes %d", enc_done->comp_bytes));
		...
		if (enc_done->comp_bytes > 0) {
			...
			libxrdp_fastpath_send_surface(self->wm->session,
					...
					enc_done->comp_bytes, x, y, x + cx, y + cy, 32,
					self->encoder->codec_id, cx, cy);
		...
		if (enc_done->last) {
			...
			if (use_frame_acks == 0) {
				self->mod->mod_frame_ack(self->mod, enc_done->enc->flags,
						enc_done->enc->frame_id);
			} else {
#if 1
				ex = self->wm->client_info->max_unacknowledged_frame_count;
				if (self->encoder->frame_id_client + ex
						> self->encoder->frame_id_server) {
					if (self->encoder->frame_id_server
							> self->encoder->frame_id_server_sent) {
						LLOGLN(10,
								("xrdp_mm_check_wait_objs: 1 -- %d", self->encoder->frame_id_server));
						self->encoder->frame_id_server_sent =
								self->encoder->frame_id_server;
...
if (self->encoder->frame_id_server > self->encoder->frame_id_server_sent) {
	LLOGLN(10,
			("xrdp_mm_frame_ack: frame_id_server %d", self->encoder->frame_id_server));
...
/*****************************************************************************/
int DEFAULT_CC
server_screen_blt(struct xrdp_mod *mod, int x, int y, int cx, int cy, int srcx,
int srcy) {
struct xrdp_wm *wm;
struct xrdp_painter *p;
...
char* data, int width, int height, int srcx, int srcy, int bpp) {
struct xrdp_wm* wm;
struct xrdp_bitmap* b;
struct xrdp_painter* p;
...
int srcrepeat, int* srctransform, int mskflags, int mskidx, int mskformat,
int mskwidth, int mskrepeat, int op, int srcx, int srcy, int mskx, int msky,
...
if (b != 0) {
xrdp_painter_composite(p, b, srcformat, srcwidth, srcrepeat, wm->target_surface,
		srctransform, mskflags, msk, mskformat, mskwidth, mskrepeat, op, srcx,
		srcy, mskx, msky, dstx, dsty, width, height, dstformat);
} else {
...
int num_crects, short *crects, char *data, int width, int height, int flags,
int frame_id) {
struct xrdp_wm* wm;
struct xrdp_mm* mm;
struct xrdp_painter* p;
...
enc_data->drects = (short *) g_malloc(sizeof(short) * num_drects * 4, 0);
if (enc_data->drects == 0) {
	g_free(enc_data);
	return 1;
}

enc_data->crects = (short *) g_malloc(sizeof(short) * num_crects * 4, 0);
...
/*****************************************************************************/
int DEFAULT_CC
server_set_pointer_ex(struct xrdp_mod *mod, int x, int y, char *data,
char *mask, int bpp) {
struct xrdp_wm *wm;
...
/*****************************************************************************/
int DEFAULT_CC
server_set_brush(struct xrdp_mod *mod, int x_orgin, int y_orgin, int style,
char *pattern) {
struct xrdp_painter *p;
...
/*****************************************************************************/
int DEFAULT_CC
server_add_char(struct xrdp_mod *mod, int font, int charactor, int offset,
int baseline, int width, int height, char *data) {
...
return libxrdp_orders_send_font(((struct xrdp_wm *) mod->wm)->session, &fi,
	font, charactor);
}

/*****************************************************************************/
int DEFAULT_CC
server_draw_text(struct xrdp_mod *mod, int font, int flags, int mixmode,
int clip_left, int clip_top, int clip_right, int clip_bottom, int box_left,
int box_top, int box_right, int box_bottom, int x, int y, char *data,
int data_len) {
struct xrdp_wm *wm;
struct xrdp_painter *p;
...
wm = (struct xrdp_wm *) (mod->wm);
return xrdp_painter_draw_text2(p, wm->target_surface, font, flags, mixmode,
	clip_left, clip_top, clip_right, clip_bottom, box_left, box_top, box_right,
	box_bottom, x, y, data, data_len);
}
...
if (wm->client_info->width == width && wm->client_info->height == height
...
if (file_read_section(fd, "channels", names, values) == 0) {
	...
	log_message(LOG_LEVEL_ERROR,
			"Failure reading channel section of configuration");
}
 ...
 * return 1 on success 0 on failure */
int update_allowed_channel_names(struct xrdp_wm *wm, struct list *names,
struct list *values) {
int ret = 1;
int index;
int oldindex;
...
if ((name != 0)
		&& (g_strncmp(name, CHANNEL_NAME_PREFIX, g_strlen(CHANNEL_NAME_PREFIX))
				== 0)) {
...
log_message(LOG_LEVEL_INFO, "This channel is disabled (not in List): %s",
		inName);
}
	...
	if (error == 0) {
		...
		if (is_channel_enabled(channelname, names, values)) {
			log_message(LOG_LEVEL_INFO,
					"The following channel is allowed: %s (%d)", channelname,
					index);
			...
			if (allowindex >= MAX_NR_CHANNELS) {
				log_message(LOG_LEVEL_ALWAYS,
						"Programming error in is_channel_allowed");
				error = 1; /* end loop */
			}
		} else {
			log_message(LOG_LEVEL_INFO,
					"The following channel is not allowed: %s (%d)",
					channelname, index);
		}
...
log_message(LOG_LEVEL_DEBUG,
		"The allow channel list now initialized for this session");
...
/*****************************************************************************/
int DEFAULT_CC
server_send_to_channel(struct xrdp_mod *mod, int channel_id, char *data,
...
/*****************************************************************************/
int DEFAULT_CC
server_create_os_surface(struct xrdp_mod *mod, int rdpindex, int width,
int height) {
struct xrdp_wm *wm;
...
if (error != 0) {
log_message(LOG_LEVEL_ERROR,
		"server_create_os_surface: xrdp_cache_add_os_bitmap failed");
return 1;
}
...
/*****************************************************************************/
int DEFAULT_CC
server_create_os_surface_bpp(struct xrdp_mod* mod, int rdpindex, int width,
int height, int bpp) {
struct xrdp_wm* wm;
struct xrdp_bitmap* bitmap;
int error;
...
log_message(LOG_LEVEL_ERROR, "server_switch_os_surface: error finding id %d",
		rdpindex);
}
...
log_message(LOG_LEVEL_ERROR, "server_paint_rect_os: error finding id %d",
		rdpindex);
}
...
/*****************************************************************************/
int DEFAULT_CC
server_window_cached_icon(struct xrdp_mod *mod, int window_id, int cache_entry,
int cache_id, int flags) {
struct xrdp_wm *wm;

wm = (struct xrdp_wm *) (mod->wm);
return libxrdp_window_cached_icon(wm->session, window_id, cache_entry, cache_id,
	flags);
}
...
/*****************************************************************************/
int DEFAULT_CC
server_add_char_alpha(struct xrdp_mod* mod, int font, int charactor, int offset,
int baseline, int width, int height, char* data) {
struct xrdp_font_char fi;
...
return libxrdp_orders_send_font(((struct xrdp_wm*) mod->wm)->session, &fi, font,
	charactor);
}
