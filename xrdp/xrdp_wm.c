...
xrdp_wm_create(struct xrdp_process *owner, struct xrdp_client_info *client_info) {
	...
	pid = g_getpid();
	g_snprintf(event_name, 255, "xrdp_%8.8x_wm_login_mode_event_%8.8x", pid,
			owner->session_id);
...
/*****************************************************************************/
int APP_CC
xrdp_wm_send_bitmap(struct xrdp_wm *self, struct xrdp_bitmap *bitmap, int x,
		int y, int cx, int cy) {
...
xrdp_wm_load_pointer(struct xrdp_wm *self, char *file_name, char *data,
	...
	if (!g_file_exist(file_name)) {
		log_message(LOG_LEVEL_ERROR,
				"xrdp_wm_load_pointer: error pointer file [%s] does not exist",
				file_name);
		return 1;
	}
	...
	if (fd < 0) {
		log_message(LOG_LEVEL_ERROR,
				"xrdp_wm_load_pointer: error loading pointer from file [%s]",
				file_name);
		xstream_free(fs);
		return 1;
	}
...
/*****************************************************************************/
int APP_CC
xrdp_wm_send_pointer(struct xrdp_wm *self, int cache_idx, char *data,
		char *mask, int x, int y, int bpp) {
...
xrdp_wm_load_static_colors_plus(struct xrdp_wm *self, char *autorun_name) {
	...
	if (fd > 0) {
			...
			for (index = 0; index < names->count; index++) {
				...
				if (val != 0) {
					if (g_strcasecmp(val, "black") == 0) {
						...
						self->black = HCOLOR(self->screen->bpp,
								xrdp_wm_htoi(val));
					} else if (g_strcasecmp(val, "grey") == 0) {
						val = (char *) list_get_item(values, index);
						self->grey = HCOLOR(self->screen->bpp,
								xrdp_wm_htoi(val));
					} else if (g_strcasecmp(val, "dark_grey") == 0) {
						val = (char *) list_get_item(values, index);
						self->dark_grey = HCOLOR(self->screen->bpp,
								xrdp_wm_htoi(val));
					} else if (g_strcasecmp(val, "blue") == 0) {
						val = (char *) list_get_item(values, index);
						self->blue = HCOLOR(self->screen->bpp,
								xrdp_wm_htoi(val));
					} else if (g_strcasecmp(val, "dark_blue") == 0) {
						val = (char *) list_get_item(values, index);
						self->dark_blue = HCOLOR(self->screen->bpp,
								xrdp_wm_htoi(val));
					} else if (g_strcasecmp(val, "white") == 0) {
						val = (char *) list_get_item(values, index);
						self->white = HCOLOR(self->screen->bpp,
								xrdp_wm_htoi(val));
					} else if (g_strcasecmp(val, "red") == 0) {
						val = (char *) list_get_item(values, index);
						self->red = HCOLOR(self->screen->bpp,
								xrdp_wm_htoi(val));
					} else if (g_strcasecmp(val, "green") == 0) {
						val = (char *) list_get_item(values, index);
						self->green = HCOLOR(self->screen->bpp,
								xrdp_wm_htoi(val));
					} else if (g_strcasecmp(val, "background") == 0) {
						val = (char *) list_get_item(values, index);
						self->background = HCOLOR(self->screen->bpp,
								xrdp_wm_htoi(val));
		...
		log_message(LOG_LEVEL_ERROR,
				"xrdp_wm_load_static_colors: Could not read xrdp.ini file %s",
				cfg_file);
	}
...
xrdp_wm_load_static_pointers(struct xrdp_wm *self) {
	...
	xrdp_wm_load_pointer(self, file_path, pointer_item.data, pointer_item.mask,
...
int DEFAULT_CC
xrdp_wm_notify(struct xrdp_bitmap *wnd, struct xrdp_bitmap *sender, int msg,
	...
	if (msg == 1) /* click */
	{
		...
		} else if (sender->id == 4) /* resume button */
		{
			xrdp_wm_set_login_mode(wnd->wm, LOGIN_MODE_CONNECTING_TO_DISPLAY);
		} else if (sender->id == 5) /* new session button */
		{
			xrdp_wm_set_login_mode(wnd->wm,
			LOGIN_MODE_CONNECTING_TO_DISPLAY_WITH_NEW_SESSION);
		}
	...
	} else if (msg == 100) /* modal result is done */
	{
		...
		if (i >= 0) {
			b = (struct xrdp_bitmap *) list_get_item(
					wnd->wm->screen->child_list, i);
...
int APP_CC xrdp_wm_init_session_window(struct xrdp_wm *self) {
	struct xrdp_cfg_globals *globals;
	struct xrdp_bitmap *but;
	globals = &self->xrdp_config->cfg_globals;

	self->session_window = xrdp_bitmap_create(302, 100, self->screen->bpp,
	WND_TYPE_WND, self);
	list_add_item(self->screen->child_list, (long) self->session_window);
	self->session_window->parent = self->screen;
	self->session_window->owner = self->screen;
	self->session_window->bg_color = globals->ls_bg_color;

	// Center window on screen
	self->session_window->left = self->screen->width / 2
			- self->session_window->width / 2;

	self->session_window->top = self->screen->height / 2
			- self->session_window->height / 2;
//
	self->session_window->notify = xrdp_wm_notify;

	set_string(&self->session_window->caption1, "Reconnexion");
	//
	/* label */
	but = xrdp_bitmap_create(280, DEFAULT_EDIT_H, self->screen->bpp,
	WND_TYPE_LABEL, self);
	list_add_item(self->session_window->child_list, (long) but);
	but->parent = self->session_window;
	but->owner = self->session_window;
	but->left = 10;
	but->top = 30;
	set_string(&but->caption1, "Une session est déjà ouverte.");

	/* OK button */
	but = xrdp_bitmap_create(90, globals->ls_btn_ok_height, self->screen->bpp,
	...
	but->top = 62;
	but->id = 4;
	set_string(&but->caption1, "Reprendre");
	but->tab_stop = 1;
	self->session_window->default_button = but;
	// Nouvelle session
	but = xrdp_bitmap_create(160, globals->ls_btn_ok_height, self->screen->bpp,
	WND_TYPE_BUTTON, self);
	list_add_item(self->session_window->child_list, (long) but);
	but->parent = self->session_window;
	but->owner = self->session_window;
	...
	but->top = 62;
	but->id = 5;
	set_string(&but->caption1, "Nouvelle session");
	but->tab_stop = 1;
	self->session_window->default_button = but;
...
//	xrdp_login_wnd_create(self);
	...
	xrdp_wm_set_focused(self, self->session_window);
...
xrdp_wm_init(struct xrdp_wm *self) {
	...
	self->screen->bg_color =
			self->xrdp_config->cfg_globals.ls_top_window_bg_color;
	...
	if (self->session->client_info->rdp_autologin) {
		...
		g_snprintf(cfg_file, 255, "%s/xrdp.ini", XRDP_CFG_PATH);
		log_info("loading configuration file %s", cfg_file);
		...
		if (fd != -1) {
			...
			if (file_read_section(fd, section_name, names, values) == 0) {
					...
					if (g_strncmp("password", q, 255) == 0) {
						...
						if (g_strncmp("ask", r, 3) == 0) {
							if (self->session->client_info->domain[0] == '_') {
								g_strncpy(param,
										&self->session->client_info->domain[1],
										255);
								r = param;
							}
				...
				g_writeln(
						"   xrdp_wm_init: file_read_section returned non-zero, requested section not found in xrdp.ini");
				xrdp_wm_log_msg(self,
						"ERROR: The requested xrdp module not found in xrdp.ini,"
			...
			log_message(LOG_LEVEL_ERROR,
					"xrdp_wm_init: Could not read xrdp.ini file %s", cfg_file);
		}
	} else {
		g_writeln(
		...
		xrdp_wm_set_login_mode(self, LOGIN_MODE_LOGIN_WINDOW_READY);
	}
...
int APP_CC
xrdp_wm_get_vis_region(struct xrdp_wm *self, struct xrdp_bitmap *bitmap, int x,
		int y, int cx, int cy, struct xrdp_region *region, int clip_children) {
	int i;
	...
	if (clip_children) {
		...
		for (i = 0; i < self->screen->child_list->count; i++) {
			p = (struct xrdp_bitmap *) list_get_item(self->screen->child_list,
					i);
...
static int APP_CC
xrdp_wm_bitblt(struct xrdp_wm *self, struct xrdp_bitmap *dst, int dx, int dy,
		struct xrdp_bitmap *src, int sx, int sy, int sw, int sh, int rop) {
	//  int i;
	//  int line_size;
	//  int Bpp;
	//  char* s;
	...
	if (self->screen == dst && self->screen == src) {
		...
		libxrdp_orders_screen_blt(self->session, dx, dy, sw, sh, sx, sy, rop,
				0);
...
/*****************************************************************************/
static int APP_CC
xrdp_wm_move_window(struct xrdp_wm *self, struct xrdp_bitmap *wnd, int dx,
		int dy) {
	...
	if (xrdp_wm_is_rect_vis(self, wnd, &rect1)) {
		...
		if (xrdp_wm_is_rect_vis(self, wnd, &rect2)) {
			...
			xrdp_wm_bitblt(self, self->screen, wnd->left + dx, wnd->top + dy,
					self->screen, wnd->left, wnd->top, wnd->width, wnd->height,
					0xcc);
			wnd->left += dx;
			wnd->top += dy;
...
xrdp_wm_undraw_dragging_box(struct xrdp_wm *self, int do_begin_end) {
	...
	if (self->dragging) {
		if (self->draggingxorstate) {
			...
			xrdp_wm_xor_pat(self, boxx, boxy, self->draggingcx,
					self->draggingcy);
	...
	if (b == 0) /* if b is null, the movement must be over the screen */
	{
		...
		if (self->mm->mod != 0) /* if screen is mod controlled */
		{
			if (self->mm->mod->mod_event != 0) {
				self->mm->mod->mod_event(self->mm->mod, WM_MOUSEMOVE, x, y, 0,
						0);
			}
		}
	}
...
xrdp_wm_mouse_click(struct xrdp_wm *self, int x, int y, int but, int down) {
	...
	if (self->dragging && but == 1 && !down && self->dragging_window != 0) {
		...
		xrdp_wm_move_window(self, self->dragging_window, newx - oldx,
				newy - oldy);
	...
	if (control == 0) {
		if (self->mm->mod != 0) /* if screen is mod controlled */
		{
			if (self->mm->mod->mod_event != 0) {
				if (but == 1 && down) {
					self->mm->mod->mod_event(self->mm->mod, WM_LBUTTONDOWN, x,
							y, 0, 0);
				} else if (but == 1 && !down) {
					self->mm->mod->mod_event(self->mm->mod, WM_LBUTTONUP, x, y,
							0, 0);
				}

				if (but == 2 && down) {
					self->mm->mod->mod_event(self->mm->mod, WM_RBUTTONDOWN, x,
							y, 0, 0);
				} else if (but == 2 && !down) {
					self->mm->mod->mod_event(self->mm->mod, WM_RBUTTONUP, x, y,
							0, 0);
				}

				if (but == 3 && down) {
					self->mm->mod->mod_event(self->mm->mod, WM_BUTTON3DOWN, x,
							y, 0, 0);
				} else if (but == 3 && !down) {
					self->mm->mod->mod_event(self->mm->mod, WM_BUTTON3UP, x, y,
							0, 0);
				}
				...
				if (but == 6 && down) {
					self->mm->mod->mod_event(self->mm->mod, WM_BUTTON6DOWN, x,
							y, 0, 0);
				} else if (but == 6 && !down) {
					self->mm->mod->mod_event(self->mm->mod, WM_BUTTON6UP, x, y,
							0, 0);
				}
				if (but == 7 && down) {
					self->mm->mod->mod_event(self->mm->mod, WM_BUTTON7DOWN, x,
							y, 0, 0);
				} else if (but == 7 && !down) {
					self->mm->mod->mod_event(self->mm->mod, WM_BUTTON7UP, x, y,
							0, 0);
				}
			}
		}
	...
	if (control != 0) {
		...
		if ((control->type == WND_TYPE_BUTTON || control->type == WND_TYPE_COMBO)
			...
			if (control->parent != 0) {
				...
				|| control->type == WND_TYPE_COMBO) && but == 1 && down) {
...
xrdp_wm_key(struct xrdp_wm *self, int device_flags, int scan_code) {
	...
	if (self->mm->mod != 0) {
		if (self->mm->mod->mod_event != 0) {
			ki = get_key_info_from_scan_code(device_flags, scan_code,
					self->keys, self->caps_lock, self->num_lock,
		...
		xrdp_bitmap_def_proc(self->focused_window, msg, scan_code,
				device_flags);
	}
...
/*****************************************************************************/
static int APP_CC
xrdp_wm_process_input_mouse(struct xrdp_wm *self, int device_flags, int x,
		int y) {
	// DEBUG("mouse event flags %4.4x  (%d,%d)", device_flags, x, y);
...
/*****************************************************************************/
static int APP_CC
xrdp_wm_process_input_mousex(struct xrdp_wm* self, int device_flags, int x,
		int y) {
...
/* param1 = MAKELONG(channel_id, flags)
 ...
 param4 = total size */
static int APP_CC
xrdp_wm_process_channel_data(struct xrdp_wm *self, tbus param1, tbus param2,
	...
	if (self->mm->mod != 0) {
		...
		if (is_channel_allowed(self, chanid)) {
			if (self->mm->usechansrv) {
				...
				if (self->mm->mod->mod_event != 0) {
					rv = self->mm->mod->mod_event(self->mm->mod, 0x5555, param1,
...
xrdp_wm_login_mode_changed(struct xrdp_wm *self) {
	...
	if (self->login_mode == LOGIN_MODE_INITIALISING_LOGIN_WINDOW) {
		log_info("login mode is LOGIN_MODE_INITIALISING_LOGIN_WINDOW");
		xrdp_wm_set_login_mode(self, LOGIN_MODE_LOGIN_WINDOW_READY);
		list_clear(self->log);
	...
	if (self->login_mode == LOGIN_MODE_INITIALISING_SESSION_WINDOW) {
		log_info("login mode is LOGIN_MODE_INITIALISING_SESSION_WINDOW");
		xrdp_wm_set_login_mode(self, LOGIN_MODE_SESSION_WINDOW_READY);
		log_info("list clear");
		list_clear(self->log);
		log_info("delete childs");
		xrdp_wm_delete_all_childs(self);
		self->dragging = 1;
		log_info("session_window");
		xrdp_wm_init_session_window(self);
	} else if (self->login_mode == LOGIN_MODE_CHECKING_SESSION) {
		log_info("login mode is LOGIN_MODE_CHECKING_SESSION");
		int messageSent = xrdp_mm_connect(self->mm, 1, 0);
		if (!messageSent) {
			log_error("error while sending message to sesman");
		}
		xrdp_wm_set_login_mode(self, LOGIN_MODE_CHECKING_SESSION_SENT);

//		if (!oldSessionFound) {
//			log_info("no previsous session found, creating a new one");
//			if (xrdp_mm_connect(self->mm,0) == 0) {
...
//			}
//		}else{
//			log_info("previous session found! Asking user if we have to resume or create a new one");
//		}

	} else if (self->login_mode == LOGIN_MODE_CONNECTING_TO_DISPLAY) {
		log_info("login mode is LOGIN_MODE_CONNECTING_TO_DISPLAY");
		int messageSent = xrdp_mm_connect(self->mm, 0, 0);
		if (!messageSent) {
			log_error("error while sending message to sesman");
		}
		xrdp_wm_delete_all_childs(self);
		self->dragging = 0;
		xrdp_wm_set_login_mode(self, LOGIN_MODE_CONNECTED_TO_DISPLAY);

	} else if (self->login_mode
			== LOGIN_MODE_CONNECTING_TO_DISPLAY_WITH_NEW_SESSION) {
		log_info("login mode is LOGIN_MODE_CONNECTING_TO_DISPLAY");
		int messageSent = xrdp_mm_connect(self->mm, 0, 1);
		if (!messageSent) {
			log_error("error while sending message to sesman");
		}
		xrdp_wm_delete_all_childs(self);
	...
	} else if (self->login_mode == LOGIN_MODE_CLOSING) {
		log_info("login mode is LOGIN_MODE_CLOSING");
		xrdp_wm_delete_all_childs(self);
		self->dragging = 0;
		xrdp_wm_set_login_mode(self, LOGIN_MODE_CLOSED);
	} else {
		log_info("unknown state : %d", self->login_mode);
	}
...
xrdp_wm_log_wnd_notify(struct xrdp_bitmap *wnd, struct xrdp_bitmap *sender,
...
void add_string_to_logwindow(char *msg, struct list *log) {
	...
	} while ((processedlen < g_strlen(msg))
			&& (processedlen < DEFAULT_STRING_LEN));
}
...
/*****************************************************************************/
int APP_CC
xrdp_wm_get_wait_objs(struct xrdp_wm *self, tbus *robjs, int *rc, tbus *wobjs,
		int *wc, int *timeout) {
	int i;
...
xrdp_wm_set_login_mode(struct xrdp_wm *self, int login_mode) {
	...
	log_info("set login mode : %d", login_mode);
