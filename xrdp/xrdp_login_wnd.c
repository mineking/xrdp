...
xrdp_wm_login_help_notify(struct xrdp_bitmap *wnd, struct xrdp_bitmap *sender,
	...
	} else if (msg == WM_PAINT) /* 3 */
	{
		...
		if (p != 0) {
			...
			xrdp_painter_draw_text(p, wnd, 10, 30,
...
the username edit box.");
			xrdp_painter_draw_text(p, wnd, 10, 94,
					"Enter the password in \
the password edit box.");
			xrdp_painter_draw_text(p, wnd, 10, 110,
...
administrator if you are");
			xrdp_painter_draw_text(p, wnd, 10, 174,
					"having problems \
logging on.");
		}
	}
...
xrdp_wm_delete_all_childs(struct xrdp_wm *self) {
	...
	for (index = self->screen->child_list->count - 1; index >= 0; index--) {
		b = (struct xrdp_bitmap *) list_get_item(self->screen->child_list,
				index);
...
set_mod_data_item(struct xrdp_mod_data *mod, char *name, char *value) {
	...
	for (index = 0; index < mod->names->count; index++) {
		if (g_strncmp(name, (char *) list_get_item(mod->names, index), 255)
				== 0) {
...
xrdp_wm_help_clicked(struct xrdp_bitmap *wnd) {
	...
	/* create help screen */
	help = xrdp_bitmap_create(DEFAULT_WND_HELP_W, DEFAULT_WND_HELP_H,
			wnd->wm->screen->bpp,
			WND_TYPE_WND, wnd->wm);
...
xrdp_wm_ok_clicked(struct xrdp_bitmap *wnd) {
	...
	if (combo != 0) {
		mod_data = (struct xrdp_mod_data *) list_get_item(combo->data_list,
				combo->item_index);
		...
		if (mod_data != 0) {
			...
			xrdp_wm_set_login_mode(wm, LOGIN_MODE_CHECKING_SESSION);
		}
	} else {
		log_message(LOG_LEVEL_ERROR,
				"Combo is 0 - potential programming error");
	}
...
xrdp_wm_show_edits(struct xrdp_wm *self, struct xrdp_bitmap *combo) {
	...
	insert_index = list_index_of(self->login_window->child_list, (long) combo); /* find combo in the list */
	insert_index++;
	mod = (struct xrdp_mod_data *) list_get_item(combo->data_list,
			combo->item_index);
	...
	if (mod != 0) {
		...
		for (index = 0; index < mod->names->count; index++) {
			...
			if (g_strncmp("ask", value, 3) == 0) {
				...
				b->top = globals->ls_input_y_pos + DEFAULT_COMBO_H + 5
						+ (DEFAULT_EDIT_H + 5) * count;
				b->id = 100 + 2 * count;
				...
				if (g_strncmp(name, "username", 255) == 0
					...
					g_strncpy(b->caption1, self->session->client_info->username,
							255);
					b->edit_pos = g_mbstowcs(0, b->caption1, 0);
				...
				if ((g_strncmp(name, "password", 255) == 0)
						|| (g_strncmp(name, "pampassword", 255) == 0))
#else
						if (g_strncmp(name, "password", 255) == 0)
#endif
						{
...
xrdp_wm_login_fill_in_combo(struct xrdp_wm *self, struct xrdp_bitmap *b) {
	...
	for (i = 0; i < sections->count; i++) {
		...
		if ((g_strncmp(p, "globals", 255) == 0)
			...
			mod_data = (struct xrdp_mod_data *) g_malloc(
					sizeof(struct xrdp_mod_data), 1);
...
xrdp_login_wnd_create(struct xrdp_wm *self) {
	...
	/* draw login window */
	self->login_window = xrdp_bitmap_create(log_width, log_height,
			self->screen->bpp,
			WND_TYPE_WND, self);
	...
	self->login_window->notify = xrdp_wm_notify;
	...
	if (regular) {
		...
		if (globals->ls_background_image[0] != 0 && self->screen->bpp > 8) {
			...
			but = xrdp_bitmap_create(4, 4, self->screen->bpp, WND_TYPE_IMAGE,
					self);
			g_snprintf(fileName, 255, "%s/%s", XRDP_SHARE_PATH,
					globals->ls_background_image);
			log_message(LOG_LEVEL_DEBUG,
					"We try to load the following background file: %s",
					fileName);
		...
		if (self->screen->bpp <= 8)
			g_snprintf(globals->ls_logo_filename, 255, "%s/ad256.bmp",
			XRDP_SHARE_PATH);
	...
	/* OK button */
	but = xrdp_bitmap_create(globals->ls_btn_ok_width,
			globals->ls_btn_ok_height, self->screen->bpp, WND_TYPE_BUTTON,
			self);
	list_add_item(self->login_window->child_list, (long) but);
	but->parent = self->login_window;
	but->owner = self->login_window;
	...
	combo->item_index = xrdp_wm_parse_domain_information(
			self->session->client_info->domain, combo->data_list->count, 1,
