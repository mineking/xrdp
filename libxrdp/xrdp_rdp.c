		...
		if (g_strcasecmp(item, "bitmap_cache") == 0) {
			...
			if (g_strcasecmp(value, "none") == 0) {
				...
				log_message(LOG_LEVEL_ALWAYS,
						"Warning: Your configured crypt level is"
			...
			if (g_strcasecmp(value, "output") == 0) {
				...
				log_message(LOG_LEVEL_ALWAYS,
						"Warning: Your configured fastpath level is "
			...
			if (g_strcasecmp(value, "rdp") == 0) {
				...
				log_message(LOG_LEVEL_ALWAYS,
						"Warning: Your configured security layer is "
			...
			if (value[0] != '/') {
				...
				g_snprintf(client_info->certificate, 1023, "%s/cert.pem",
						XRDP_CFG_PATH);
				log_message(LOG_LEVEL_ALWAYS,
						"WARNING: Invalid x.509 certificate path defined, "
								"default path will be used: %s",
			...
			if (value[0] != '/') {
				...
				g_snprintf(client_info->key_file, 1023, "%s/key.pem",
						XRDP_CFG_PATH);
				log_message(LOG_LEVEL_WARNING,
						"Invalid X.509 certificate path defined, "
								"default path will be used: %s",
	...
	if (s->next_packet == 0 || s->next_packet >= s->end) {
		...
		if ((chan != MCS_GLOBAL_CHANNEL) && (chan > 0)) {
			if (chan > MCS_GLOBAL_CHANNEL) {
				if (xrdp_channel_process(self->sec_layer->chan_layer, s, chan)
						!= 0) {
				...
				if (chan != 1) {
					g_writeln(
							"Wrong channel Id to be handled by xrdp_channel_process %d",
							chan);
				}
			}
...
xrdp_rdp_send_data(struct xrdp_rdp *self, struct stream *s, int data_pdu_type) {
	...
	//		DEBUG(
	//				"mppc_encode ok flags 0x%x bytes_in_opb %d historyOffset %d " "tocomplen %d", mppc_enc->flags, mppc_enc->bytes_in_opb, mppc_enc->historyOffset, tocomplen);
			...
			LLOGLN(10,
					("xrdp_rdp_send_data: mppc_encode not ok " "type %d flags %d", mppc_enc->protocol_type, mppc_enc->flags));
		}
	}
...
xrdp_rdp_send_fastpath(struct xrdp_rdp *self, struct stream *s,
	...
	while (cont) {
		...
		LLOGLN(10,
				("xrdp_rdp_send_fastpath: no_comp_len %d fragmentation %d", no_comp_len, fragmentation));
		if ((compression != 0) && (no_comp_len > header_bytes + 16)) {
			...
			if (compress_rdp(mppc_enc, (tui8 *) (frag_s.p + header_bytes),
				...
				LLOGLN(10,
						("xrdp_rdp_send_fastpath: no_comp_len %d " "comp_len %d", no_comp_len, comp_len));
				send_len = comp_len;
				...
				LLOGLN(10,
						("xrdp_rdp_send_fastpath: mppc_encode not ok " "type %d flags %d", mppc_enc->protocol_type, mppc_enc->flags));
			}
		}
		updateHeader = (updateCode & 15) | ((fragmentation & 3) << 4)
				| ((compression & 3) << 6);
		...
		LLOGLN(10,
				("xrdp_rdp_send_fastpath: header_bytes: %d send legnth: %d",header_bytes ,send_len));
		out_uint16_le(&send_s, send_len); // substract header_bytes ??
...
xrdp_rdp_send_data_update_sync(struct xrdp_rdp *self) {
	...
	make_stream(s);
	init_stream(s, 8192);
	DEBUG("\n\nin xrdp_rdp_send_data_update_sync");
	...
	{
		LLOGLN(10, ("xrdp_rdp_send_data_update_sync: data"));
	...
	if (self->client_info.use_fast_path & 1) /* fastpath output supported */
	{
		LLOGLN(10, ("xrdp_rdp_send_data_update_sync: xrdp_rdp_send_fastpath"));
	...
	{
		LLOGLN(10, ("xrdp_rdp_send_data_update_sync: xrdp_rdp_send_data"));
...
xrdp_rdp_process_data_input(struct xrdp_rdp *self, struct stream *s) {
	...
	//	DEBUG(
		//		"xrdp_rdp_process_data_input event %4.4x flags %4.4x param1 %d " "param2 %d time %d", msg_type, device_flags, param1, param2, time);
...
xrdp_rdp_process_data_control(struct xrdp_rdp *self, struct stream *s) {
	...
	if (action == RDP_CTL_REQUEST_CONTROL) {
		...
		DEBUG
				("xrdp_rdp_process_data_control calling xrdp_rdp_send_synchronise");
...
xrdp_rdp_process_frame_ack(struct xrdp_rdp *self, struct stream *s) {
	...
	if (self->session->callback != 0) {
		/* call to xrdp_wm.c : callback */
		self->session->callback(self->session->id, 0x5557, frame_id, 0, 0, 0);
	}
	return 0;
}
...
xrdp_rdp_process_data(struct xrdp_rdp *self, struct stream *s) {
	...
	case 56: /* PDUTYPE2_FRAME_ACKNOWLEDGE 0x38 */
		...
		break;
	default:
		log_error("unknown in xrdp_rdp_process_data %d", data_type);
		break;
	}
