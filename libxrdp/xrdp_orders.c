...
#ifdef DEBUG_ORDER
			log_order("ending %d orders", self->order_count);
#endif
			self->order_count_ptr[0] = self->order_count;
			self->order_count_ptr[1] = self->order_count >> 8;
			self->order_count = 0;
			if (self->rdp_layer->client_info.use_fast_path & 1) {
				if (xrdp_rdp_send_fastpath(self->rdp_layer, self->out_s, 0)
						!= 0) {
					rv = 1;
				}
			} else {
...
xrdp_orders_force_send(struct xrdp_orders *self) {
	...
	if ((self->order_level > 0) && (self->order_count > 0)) {
		s_mark_end(self->out_s);
#ifdef DEBUG_ORDER
		log_order("force_send sending %d orders", self->order_count);
#endif
		self->order_count_ptr[0] = self->order_count;
		self->order_count_ptr[1] = self->order_count >> 8;
		if (self->rdp_layer->client_info.use_fast_path & 1) {
			if (xrdp_rdp_send_fastpath(self->rdp_layer, self->out_s,
...
xrdp_orders_check(struct xrdp_orders *self, int max_size) {
	...
	if (size < 0) {
		log_error("error in xrdp_orders_check, size too small, its %d", size);
		return 1;
	}
	if (size > max_packet_size) {
		...
		log_error("error in xrdp_orders_check, size too big, its %d", size);
...
/* returns error */
static int APP_CC
xrdp_order_pack_small_or_tiny(struct xrdp_orders *self, char *order_flags_ptr,
		int orders_flags, char *present_ptr, int present, int present_size) {
	int move_up_count = 0;
	int index = 0;
	int size = 0;
	int keep_looking = 1;
...
xrdp_orders_rect(struct xrdp_orders *self, int x, int y, int cx, int cy,
...
#ifdef DEBUG_ORDER
	log_order("rectangle : %d,%d %dx%d color:%d", x, y, cx, cy, color);
#endif
	...
	if (rect != 0) {
		...
		if (x < rect->left || y < rect->top || x + cx > rect->right
				|| y + cy > rect->bottom) {
	...
	if ((color & 0xff) != (self->orders_state.rect_color & 0xff)) {
		present |= 0x10;
		self->orders_state.rect_color = (self->orders_state.rect_color
				& 0xffff00) | (color & 0xff);
...
/* max size 25 */
int APP_CC
xrdp_orders_screen_blt(struct xrdp_orders *self, int x, int y, int cx, int cy,
		int srcx, int srcy, int rop, struct xrdp_rect *rect) {
	int order_flags = 0;
...
#ifdef DEBUG_ORDER
	log_order("screen_blt : %d,%d %dx%d src:%d,%d %d", x, y, cx, cy, srcx, srcy,
			rop);
#endif
...
/* max size 39 */
int APP_CC
xrdp_orders_pat_blt(struct xrdp_orders *self, int x, int y, int cx, int cy,
		int rop, int bg_color, int fg_color, struct xrdp_brush *brush,
...
#ifdef DEBUG_ORDER
	log_order("pat_blt : %d,%d %dx%d rop:%d fColor:%d fgColor:%d  ", x, y, cx,
			cy, rop, bg_color, fg_color);
	//return 0;
#endif
...
/* max size 21 */
int APP_CC
xrdp_orders_dest_blt(struct xrdp_orders *self, int x, int y, int cx, int cy,
	...
	char *order_flags_ptr;
#ifdef DEBUG_ORDER
	log_order("dest_blt : %d,%d %dx%d rop: %d", x, y, cx, cy, rop);
#endif
...
/* max size 32 */
int APP_CC
xrdp_orders_line(struct xrdp_orders *self, int mix_mode, int startx, int starty,
		int endx, int endy, int rop, int bg_color, struct xrdp_pen *pen,
		struct xrdp_rect *rect) {
	int order_flags = 0;
...
#ifdef DEBUG_ORDER
	log_order("line : %d,%d to %d,%d rop: %d", startx, starty, endx, endy, rop);
#endif
...
/* max size  30 */
int APP_CC
xrdp_orders_mem_blt(struct xrdp_orders *self, int cache_id, int color_table,
		int x, int y, int cx, int cy, int rop, int srcx, int srcy,
	...
	char *order_flags_ptr = (char *) NULL;
#ifdef DEBUG_ORDER
	log_order("mem_blt : %d,%d %dx%d rop:%d src:%d;%d", x, y, cx, cy, rop, srcx,
			srcy);
...
xrdp_orders_composite_blt(struct xrdp_orders* self, int srcidx, int srcformat,
		int srcwidth, int srcrepeat, int* srctransform, int mskflags,
		int mskidx, int mskformat, int mskwidth, int mskrepeat, int op,
		int srcx, int srcy, int mskx, int msky, int dstx, int dsty, int width,
		int height, int dstformat, struct xrdp_rect* rect) {
	int order_flags;
	int vals[20];
	int present;
	char* present_ptr;
	char* order_flags_ptr;
#ifdef DEBUG_ORDER
	log_order("xrdp_order : composite_blt : ");
#endif
	...
	if (rect != 0) {
		/* if clip is present, still check if its needed */
		if (dstx < rect->left || dsty < rect->top || dstx + width > rect->right
				|| dsty + height > rect->bottom) {
			order_flags |= RDP_ORDER_BOUNDS;
			if (xrdp_orders_last_bounds(self, rect)) {
	...
	if (srcwidth != self->orders_state.com_blt_srcwidth) {
		...
		if (order_flags & RDP_ORDER_DELTA) {
			out_uint8(self->out_s,
					srcwidth - self->orders_state.com_blt_srcwidth);
		} else {
	...
	if (srctransform != 0) {
		...
		if (g_memcmp(&(srctransform[1]),
				&(self->orders_state.com_blt_srctransform[1]), 36) != 0) {
			present |= 0x000020;
	...
	if (mskwidth != self->orders_state.com_blt_mskwidth) {
		...
		if (order_flags & RDP_ORDER_DELTA) {
			out_uint8(self->out_s,
					mskwidth - self->orders_state.com_blt_mskwidth);
		} else {
...
/* returns error */
int APP_CC
xrdp_orders_text(struct xrdp_orders *self, int font, int flags, int mixmode,
		int fg_color, int bg_color, int clip_left, int clip_top, int clip_right,
		int clip_bottom, int box_left, int box_top, int box_right,
	...
	if (rect != 0) {
		...
		if ((box_right - box_left > 1
				&& (box_left < rect->left || box_top < rect->top
						|| box_right > rect->right || box_bottom > rect->bottom))
				|| (clip_left < rect->left || clip_top < rect->top
...
int APP_CC
xrdp_orders_send_palette(struct xrdp_orders *self, int *palette, int cache_id) {
	int order_flags;
	int len;
	int i;
#ifdef DEBUG_ORDER
	log_order("xrdp_order : send_palette :");
#endif
...
int APP_CC
xrdp_orders_send_raw_bitmap(struct xrdp_orders *self, int width, int height,
	...
	int e = 0;
#ifdef DEBUG_ORDER
	log_order("raw_bitmap : %dx%d at %d bpp ", width, height, bpp);
#endif
	if (width > 64) {
...
/* max size width * height * Bpp + 16 */
int APP_CC
xrdp_orders_send_bitmap(struct xrdp_orders *self, int width, int height,
		int bpp, char *data, int cache_id, int cache_idx) {
	int order_flags = 0;
	int len = 0;
	int bufsize = 0;
	...
	char *p = NULL;
#ifdef DEBUG_ORDER
	log_order("xrdp_order : send_bitmap :", width, height, bpp);
#endif
	if (width > 64) {
		g_writeln("error, width > 64");
		return 1;
	...
	i = height;
	if (bpp > 24) {
		lines_sending = xrdp_bitmap32_compress(data, width, height, s, bpp,
				16384, i - 1, temp_s, e, 0x10);
	} else {
	...
	if (lines_sending != height) {
		g_writeln(
				"error in xrdp_orders_send_bitmap, lines_sending(%d) != \
height(%d)",
	...
	if (!self->rdp_layer->client_info.op2) {
		...
		out_uint16_le(self->out_s, (width + e) * Bpp * height); /* final size */
	}
...
xrdp_orders_send_font(struct xrdp_orders *self,
	...
	int flags;
#ifdef DEBUG_ORDER
	log_order("xrdp_order : send_font :  ");
#endif
...
int APP_CC
xrdp_orders_send_raw_bitmap2(struct xrdp_orders *self, int width, int height,
		int bpp, char *data, int cache_id, int cache_idx) {
	int order_flags = 0;
	int len = 0;
	int bufsize = 0;
...
/* max size width * height * Bpp + 14 */
int APP_CC
xrdp_orders_send_bitmap2(struct xrdp_orders *self, int width, int height,
	...
	if (lines_sending != height) {
		g_writeln(
				"error in xrdp_orders_send_bitmap2, lines_sending(%d) != \
height(%d)",
				lines_sending, height);
		return 1;
	}
...
/*****************************************************************************/
static int xrdp_orders_send_as_jpeg(struct xrdp_orders *self, int width,
		int height, int bpp, int hints) {
	if (hints & 1) {
		return 0;
	}
...
xrdp_orders_out_v3(struct xrdp_orders *self, int cache_id, int cache_idx,
		char *buf, int bufsize, int width, int height, int bpp, int codec_id) {
	int Bpp;
	int order_flags;
	int len;
	int i;
...
/*  secondary drawing order (bitmap v3) using remotefx compression */
int APP_CC
xrdp_orders_send_bitmap3(struct xrdp_orders *self, int width, int height,
		int bpp, char *data, int cache_id, int cache_idx, int hints) {
	int e;
	int bufsize;
	int quality;
...
int APP_CC
xrdp_orders_send_brush(struct xrdp_orders *self, int width, int height, int bpp,
		int type, int size, char *data, int cache_id) {
	int order_flags = 0;
	int len = 0;
#ifdef DEBUG_ORDER
	log_order("brush :");
...
int APP_CC
xrdp_orders_send_create_os_surface(struct xrdp_orders *self, int id, int width,
		int height, struct list *del_list) {
	int order_flags;
	int cache_id;
	int flags;
	int index;
