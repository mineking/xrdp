...
libxrdp_init(tbus id, struct trans *trans) {
	...
	session->orders = xrdp_orders_create(session,
			(struct xrdp_rdp *) session->rdp);
...
libxrdp_force_read(struct trans* trans) {
	...
	g_writeln("libxrdp_force_read: trans->status : %d source: %d",
			trans->status, trans->my_source);
	s1 = trans_force_read(trans, 4);
	if (s1 != 0) {
		g_writeln(
				"libxrdp_force_read: error : trans_force_read(trans, 4) != 0 : s1:%d",
				s1);
		return 0;
	}
	bytes = libxrdp_get_pdu_bytes(s->data);
	if (bytes < 1) {
		g_writeln("libxrdp_force_read: error : bytes < 1 (%d)", bytes);
		return 0;
	}
	if (bytes > 32 * 1024) {
		g_writeln("libxrdp_force_read: error : bytes > 32 * 1024 (%d)", bytes);
		return 0;
	}

	if (trans_force_read(trans, bytes - 4) != 0) {
		g_writeln(
				"libxrdp_force_read: error : trans_force_read(trans, bytes - 4) != 0");
		return 0;
	}
	return s;
}
...
libxrdp_process_data(struct xrdp_session *session, struct stream *s) {
	...
	while ((cont || !session->up_and_running) && !term) {
		...
		case 2: /* FASTPATH_INPUT_EVENT */
			if (xrdp_fastpath_process_input_event(
					rdp->sec_layer->fastpath_layer, s) != 0) {
				DEBUG("libxrdp_process_data returned non zero");
				cont = 0;
				term = 1;
			}
...
libxrdp_send_bitmap(struct xrdp_session *session, int width, int height,
	...
	make_stream(s);
	init_stream(s, 8192);
	g_writeln("WARNING COMPRESSION DISABLED %d,%d", x, y);
	if (session->client_info->use_bitmap_comp && 0) {
		make_stream(temp_s);
		...
		while (i > 0) {
			...
			do {
				...
				lines_sending = xrdp_bitmap_compress(data, width, height, s,
						bpp, 4096 - total_bufsize, i - 1, temp_s, e);
				...
				if (bufsize > 8192) {
					g_writeln("error, compressed size too big, its %d",
							bufsize);
				}
...
/*****************************************************************************/
int EXPORT_CC
libxrdp_send_pointer(struct xrdp_session *session, int cache_idx, char *data,
...
/******************************************************************************/
int EXPORT_CC
libxrdp_orders_rect(struct xrdp_session *session, int x, int y, int cx, int cy,
		int color, struct xrdp_rect *rect) {
	return xrdp_orders_rect((struct xrdp_orders *) session->orders, x, y, cx,
			cy, color, rect);
}

/******************************************************************************/
int EXPORT_CC
libxrdp_orders_screen_blt(struct xrdp_session *session, int x, int y, int cx,
		int cy, int srcx, int srcy, int rop, struct xrdp_rect *rect) {
	return xrdp_orders_screen_blt((struct xrdp_orders *) session->orders, x, y,
			cx, cy, srcx, srcy, rop, rect);
}

/******************************************************************************/
int EXPORT_CC
libxrdp_orders_pat_blt(struct xrdp_session *session, int x, int y, int cx,
		int cy, int rop, int bg_color, int fg_color, struct xrdp_brush *brush,
		struct xrdp_rect *rect) {
	return xrdp_orders_pat_blt((struct xrdp_orders *) session->orders, x, y, cx,
			cy, rop, bg_color, fg_color, brush, rect);
}

/******************************************************************************/
int EXPORT_CC
libxrdp_orders_dest_blt(struct xrdp_session *session, int x, int y, int cx,
		int cy, int rop, struct xrdp_rect *rect) {
	return xrdp_orders_dest_blt((struct xrdp_orders *) session->orders, x, y,
			cx, cy, rop, rect);
}

/******************************************************************************/
int EXPORT_CC
libxrdp_orders_line(struct xrdp_session *session, int mix_mode, int startx,
		int starty, int endx, int endy, int rop, int bg_color,
		struct xrdp_pen *pen, struct xrdp_rect *rect) {
	return xrdp_orders_line((struct xrdp_orders *) session->orders, mix_mode,
			startx, starty, endx, endy, rop, bg_color, pen, rect);
}
...
libxrdp_orders_mem_blt(struct xrdp_session *session, int cache_id,
	...
	return xrdp_orders_mem_blt((struct xrdp_orders *) session->orders, cache_id,
			color_table, x, y, cx, cy, rop, srcx, srcy, cache_idx, rect);
}

/******************************************************************************/
int DEFAULT_CC
libxrdp_orders_composite_blt(struct xrdp_session* session, int srcidx,
		int srcformat, int srcwidth, int srcrepeat, int* srctransform,
		int mskflags, int mskidx, int mskformat, int mskwidth, int mskrepeat,
		int op, int srcx, int srcy, int mskx, int msky, int dstx, int dsty,
		int width, int height, int dstformat, struct xrdp_rect* rect) {
	return xrdp_orders_composite_blt((struct xrdp_orders*) session->orders,
			srcidx, srcformat, srcwidth, srcrepeat, srctransform, mskflags,
			mskidx, mskformat, mskwidth, mskrepeat, op, srcx, srcy, mskx, msky,
			dstx, dsty, width, height, dstformat, rect);
}

/******************************************************************************/
int EXPORT_CC
libxrdp_orders_text(struct xrdp_session *session, int font, int flags,
		int mixmode, int fg_color, int bg_color, int clip_left, int clip_top,
		int clip_right, int clip_bottom, int box_left, int box_top,
		int box_right, int box_bottom, int x, int y, char *data, int data_len,
		struct xrdp_rect *rect) {
	return xrdp_orders_text((struct xrdp_orders *) session->orders, font, flags,
			mixmode, fg_color, bg_color, clip_left, clip_top, clip_right,
			clip_bottom, box_left, box_top, box_right, box_bottom, x, y, data,
			data_len, rect);
}
...
/*****************************************************************************/
int EXPORT_CC
libxrdp_orders_send_raw_bitmap(struct xrdp_session *session, int width,
		int height, int bpp, char *data, int cache_id, int cache_idx) {
...
/*****************************************************************************/
int EXPORT_CC
libxrdp_orders_send_bitmap(struct xrdp_session *session, int width, int height,
		int bpp, char *data, int cache_id, int cache_idx) {
...
/*****************************************************************************/
int EXPORT_CC
libxrdp_reset(struct xrdp_session *session, int width, int height, int bpp) {
...
/*****************************************************************************/
int EXPORT_CC
libxrdp_orders_send_raw_bitmap2(struct xrdp_session *session, int width,
		int height, int bpp, char *data, int cache_id, int cache_idx) {
...
/*****************************************************************************/
int EXPORT_CC
libxrdp_orders_send_bitmap2(struct xrdp_session *session, int width, int height,
		int bpp, char *data, int cache_id, int cache_idx, int hints) {
	return xrdp_orders_send_bitmap2((struct xrdp_orders *) session->orders,
			width, height, bpp, data, cache_id, cache_idx, hints);
}

/*****************************************************************************/
int EXPORT_CC
libxrdp_orders_send_bitmap3(struct xrdp_session *session, int width, int height,
		int bpp, char *data, int cache_id, int cache_idx, int hints) {
...
libxrdp_query_channel(struct xrdp_session *session, int index,
	...
	if (index < 0 || index >= count) {
		log_error("libxrdp_query_channel - Channel out of range %d", index);
		return 1;
	}

	channel_item = (struct mcs_channel_item *) list_get_item(mcs->channel_list,
			index);
	...
	if (channel_name != 0) {
		...
		DEBUG(
				"libxrdp_query_channel - Channel %d name %s", index, channel_name);
	}
...
libxrdp_get_channel_id(struct xrdp_session *session, char *name) {
	...
	for (index = 0; index < count; index++) {
		channel_item = (struct mcs_channel_item *) list_get_item(
...
/*****************************************************************************/
int EXPORT_CC
libxrdp_orders_send_brush(struct xrdp_session *session, int width, int height,
		int bpp, int type, int size, char *data, int cache_id) {
	return xrdp_orders_send_brush((struct xrdp_orders *) session->orders, width,
			height, bpp, type, size, data, cache_id);
}
...
libxrdp_window_new_update(struct xrdp_session *session, int window_id,
	...
	return xrdp_orders_send_window_new_update(orders, window_id, window_state,
			flags);
}
...
libxrdp_window_icon(struct xrdp_session *session, int window_id,
		int cache_entry, int cache_id, struct rail_icon_info *icon_info,
		int flags) {
	struct xrdp_orders *orders;
...
/*****************************************************************************/
int EXPORT_CC
libxrdp_notify_new_update(struct xrdp_session *session, int window_id,
		int notify_id, struct rail_notify_state_order *notify_state, int flags) {
	struct xrdp_orders *orders;
...
/*****************************************************************************/
int DEFAULT_CC
libxrdp_notify_delete(struct xrdp_session *session, int window_id,
		int notify_id) {
	struct xrdp_orders *orders;
...
/*****************************************************************************/
int EXPORT_CC
libxrdp_codec_jpeg_compress(struct xrdp_session *session, int format,
		char *inp_data, int width, int height, int stride, int x, int y, int cx,
		int cy, int quality, char *out_data, int *io_len) {
	struct xrdp_orders *orders;
	void* jpeg_han;

	orders = (struct xrdp_orders *) (session->orders);
	jpeg_han = orders->jpeg_han;
	return xrdp_codec_jpeg_compress(jpeg_han, format, inp_data, width, height,
			stride, x, y, cx, cy, quality, out_data, io_len);
}

/*****************************************************************************/
int EXPORT_CC
libxrdp_fastpath_send_surface(struct xrdp_session *session, char* data_pad,
		int pad_bytes, int data_bytes, int destLeft, int destTop, int destRight,
		int destBottom, int bpp, int codecID, int width, int height) {
	struct stream ls;
	struct stream *s;
	struct xrdp_rdp *rdp;
	int rv;
